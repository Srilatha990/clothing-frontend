...
export default function AccountChangePassword() {
  ...
  const ChangePassWordSchema = Yup.object().shape({
    oldPassword: Yup.string().trim().matches(/^(\S+$)/g, '* This field cannot contain blankspaces').required(),
    newPassword: Yup.string().trim().matches(/^(\S+$)/g, '* This field cannot contain blankspaces').min(6, 'Password must be at least 6 characters').required('New password is required'),
    confirmPassword: Yup.string().trim().matches(/^(\S+$)/g, '* This field cannot contain blankspaces').oneOf([Yup.ref('newPassword')], 'Passwords must match')
  });
