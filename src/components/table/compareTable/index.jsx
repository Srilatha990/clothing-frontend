...
import { removeCompareProduct, resetCompareProducts } from 'src/redux/slices/compare';
...
const CompareTable = () => {
  ...
  function onRemoveCompare(productid){
    dispatch(removeCompareProduct(productid));
      window.location.reload()
  }
  function onresetCompareProducts(){
    dispatch(resetCompareProducts());
      window.location.reload()
  }
  ...
  return isLoading ? null : fetchedProducts?.data?.length ? (
    ...
    <Typography variant="a" onClick={() => onresetCompareProducts()} aria-label="Remove from All" sx={{ position: 'relative', paddingBottom: 10, cursor: 'pointer' }}><IoIosCloseCircle /> Remove All</Typography>
    </div>
